...
class manage_comments extends baseController {
    ...
    public function index() {
        ...
        view::assign('node_name', view::$language->manage_comments_title);
        $this->setProtectedLayout('manage-comments.html');
    ...
    public function delete() {
        ...
        if (!validate::isNumber($commentID)) {
            throw new memberErrorException(
                view::$language->manage_comments_error,
                view::$language->manage_comments_data_invalid
            );
        }
        ...
        $this->redirectMessage(
            SUCCESS_EXCEPTION,
            view::$language->manage_comments_success,
            view::$language->manage_comments_comment_is_deleted,
    ...
    public function edit() {
        ...
        if (!$comment = db::normalizeQuery(
            ...
            throw new memberErrorException(
                view::$language->manage_comments_error,
                view::$language->manage_comments_comment_not_found
            );
        }
        ...
        $comment['comment_text'] = preg_replace(
            '/\s*<br\s?\/?>\s*/', "\n", $comment['comment_text']
        );
        view::assign('comment', $comment);
        view::assign('page_title', view::$language->manage_comments_edit_title);
    ...
    private function saveComment($id) {
        ...
        $comment = filter::input($comment)->htmlSpecialChars()->getData();

        if (!$comment) {
            throw new memberErrorException(
                view::$language->manage_comments_error,
                view::$language->manage_comments_data_not_enough
            );
        }
        ...
        if ($author !== null) {
            ...
            $author = filter::input($author)->htmlSpecialChars()->getData();
            if (!$author) {
                throw new memberErrorException(
                    view::$language->manage_comments_error,
                    view::$language->manage_comments_data_not_enough
        ...
        $this->redirectMessage(
            SUCCESS_EXCEPTION,
            view::$language->manage_comments_success,
            view::$language->manage_comments_comment_is_edited,
            $adminToolsLink . '/manage-comments'
        );
