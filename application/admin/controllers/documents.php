...
class documents extends baseController {
    ...
    public function runBefore() {
        $this->root['page_name'] = view::$language->root_of_site;
    }
    ...
    public function branch() {
        ...
        view::assign("page_title", view::$language->document_tree);
    ...
    private function branchChildren($parent) {
        ...
        return db::query("
            ...
            GROUP BY c.id
            ORDER BY c.sort ASC, c.page_name ASC
    ...
    private function getAvailableParents($branchID, $documentID, $isAllParents) {
        ...
        $branchChildren = db::query("
            ...
            GROUP BY c.id
            ORDER BY children DESC, c.sort ASC
    ...
    private function getDynamicProperties( & $document, $newPrototypeID = null) {
        ...
        if ($newPrototypeID !== null and $document['prototype'] != $newPrototypeID) {
            ...
            $document['sys_name'] = $this->getSystemNameOfProrotype($newPrototypeID);
        ...
        if (!$props) {
            throw new systemErrorException(view::$language->error, view::$language->document_dyn_props_not_found);
        }
        ...
        if (!$isChangedPrototype and utils::isNumber($document['id'])) {
            ...
            if (!$values) {
                throw new systemErrorException(view::$language->error, view::$language->document_dyn_values_not_found);
            }
    ...
    private function assignDocumentPropertiesIntoView($document) {
        ...
        view::assign(
            "children_prototypes",
            $this->getPrototypesList($document['c_prototype'])
        );
    ...
    private function getExistsDocumentProperties($documentID) {
        ...
        $existsDocument = db::normalizeQuery("
            ...
            LEFT JOIN documents p ON p.id = d.parent_id
            LEFT JOIN prototypes pt ON pt.id = d.prototype
    ...
    private function getNewDocumentProperties($parentID) {
        ...
        if ($parentID == 0) {
            ...
            $newDocument['c_prototype']  = $defaultPrototype['id'];
         ...
         */
            ...
            $existsParent = db::normalizeQuery("
                ...
                SELECT
                    ...
                    p.page_name parent_name,
                    p.c_prototype prototype,
                    p.page_alias parent_alias,
                    pt.sys_name

                FROM documents p
                LEFT JOIN prototypes pt ON pt.id = p.c_prototype
                WHERE p.id = %u
    ...
    private function getFilteredRequiredInputData($documentID = null) {
        ...
        if (!$inputDocument['page_name'] = filter::input($inputDocument['page_name'])->textOnly()->getData()) {
        ...
        $existsChildrenPrototype = db::query("
            ...
            WHERE id = %u", $inputDocument['c_prototype']
        );
