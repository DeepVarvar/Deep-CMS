...
abstract class autorun {
    ...
    private static function init($type = autorun::RUN_BEFORE) {
        ...
        foreach (utils::glob("{$autorunDirectory}*.php") as $item) {
            ...
            if (!method_exists($component, $action)) {
                throw new systemErrorException("Autoload error", "Impossible execute {$component}::{$action}()");
            }
            ...
            if (!$checkMethod->isPublic()) {
                throw new systemErrorException("Autoload error", "Method {$action}() on $component is not public");
            }

            if (!$checkMethod->isStatic()) {
                throw new systemErrorException("Autoload error", "Method {$action}() on $component is not static");
            }
